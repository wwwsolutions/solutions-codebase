...
const targetPath =
  './libs/shared/environments/src/lib/environments/environment.stage.ts';
...
if (JSON.parse(env.ENVIRONMENT_LOGGING_ENABLED)) {
  ...
  console.log(
    colors.magenta(
      'The file `environment.stage.ts` will be written with the following content: \n'
    )
  );
...
fs.writeFile(targetPath, envConfigFile, function (err) {
  if (err) {
    ...
    console.log(
      colors.magenta(
        `environment.stage.ts file generated correctly at ${targetPath} \n`
      )
    );
  }
});
